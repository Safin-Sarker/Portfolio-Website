# Technical Skills

## AI & Machine Learning

### Core AI/ML Competencies
- **RAG (Retrieval Augmented Generation)**: Expert in building knowledge-based AI systems with vector databases (ChromaDB) and embeddings
- **Agentic AI Systems**: Building intelligent agents with LangChain and LangGraph for autonomous decision-making
- **LangChain Framework**: Orchestrating LLM applications, building chains, agents, and RAG pipelines
- **Prompt Engineering**: Advanced techniques for optimal AI outputs and context management
- **Vector Databases**: ChromaDB, embeddings, and semantic search implementation

### Generative AI
- **Stable Diffusion**: Expert in fine-tuning and deploying diffusion models
- **LoRA Fine-tuning**: Training and optimizing Low-Rank Adaptation models
- **Train LLM in LoRA**: Experience in training large language models using LoRA
- **ComfyUI**: Building and deploying AI workflows
- **Serverless AI Deployment**: Deploying AI models in production

### LLM Integration & Tools
- **OpenAI API**: GPT models integration and usage
- **Ollama**: Local LLM deployment and management
- **Azure AI Foundry**: Cloud-based AI development
- **Claude Code Agent (Anthropic)**: Completed "Claude Code: A Highly Agentic Coding Assistant" course by Anthropic
  - AI-assisted refactoring and code generation
  - Automated pull request creation, review, and modification
  - MCP Server integration for connecting external tools
  - Git worktrees for running multiple Claude sessions in parallel
  - Multi-branch development coordination
  - Complex workflow automation with agentic AI
  - Context-aware code improvements and debugging
- **GitHub Copilot**: AI-assisted code generation and development
- **ChatGPT**: Integration and API usage

### Machine Learning & Data Science
- **Python**: Primary language for ML/AI development
- **Data Analysis**: Pandas, NumPy for data manipulation
- **Machine Learning**: Scikit-learn, model training
- **Deep Learning**: Neural networks, PyTorch
- **Computer Vision**: Image processing, OpenCV
- **Model Training & Optimization**: Performance tuning, hyperparameter optimization

---

## Backend Development

### .NET Ecosystem
- **ASP.NET Core MVC**: Building web applications
- **ASP.NET Core Web API**: RESTful API development
- **C#**: Advanced programming and design patterns
- **Entity Framework**: ORM and database management
- **LINQ**: Query optimization and data manipulation

### Architecture & Design
- **Clean Architecture**: Separation of concerns, maintainable code
- **SOLID Principles**: Object-oriented design best practices
- **Design Patterns**: Repository, Factory, Dependency Injection
- **RESTful API Design**: Best practices for API development

### API Development & Integration
- **RESTful API Integration**: Consuming and integrating third-party APIs
- **API Authentication**: OAuth, JWT, API key management and security
- **Webhook Implementation**: Event-driven integrations and real-time data processing
- **API Documentation**: Swagger/OpenAPI specification and documentation best practices
- **Rate Limiting & Error Handling**: Production-ready API consumption with proper error management
- **API Testing**: Postman, integration testing, and API validation

### Other Backend Technologies
- **Flask**: Python web framework for APIs
- **Node.js**: JavaScript runtime for backend services

---

## Frontend Development

### Modern Frameworks
- **React**: Component-based UI development
- **Next.js**: Server-side rendering, static generation
- **Redux**: State management
- **RTK Query**: Efficient data fetching and caching

### Languages & Core Technologies
- **TypeScript**: Type-safe JavaScript development
- **JavaScript (ES6+)**: Modern JavaScript features
- **HTML5**: Semantic markup
- **CSS3**: Modern styling techniques

### Styling Frameworks
- **Tailwind CSS**: Utility-first CSS framework
- **Bootstrap**: Responsive design framework
- **Framer Motion**: Animation library for React

---

## Database Technologies

### Relational Databases
- **Microsoft SQL Server (MSSQL)**: Enterprise database management
- **MySQL**: Open-source relational database
- **Entity Framework**: ORM for .NET applications
- **LINQ**: Language-integrated query
- **Stored Procedures**: Database logic and optimization

### NoSQL & Caching
- **Redis**: In-memory data structure store, caching
- **ChromaDB**: Vector database for embeddings

---

## DevOps & Cloud

### Containerization & Orchestration
- **Docker**: Container creation and management
- **Docker Compose**: Multi-container applications
- **Serverless Architecture**: Event-driven, scalable applications

### Cloud Platforms

#### AWS (Amazon Web Services)
- **EC2**: Virtual servers and compute instances
- **S3**: Object storage and static hosting
- **Auto Scaling**: Automatic capacity management
- **Lambda**: Serverless compute

#### Azure (Microsoft Azure)
- **Azure VMs**: Virtual machine management
- **Azure Blob Storage**: Cloud object storage
- **Azure App Services**: Web app hosting
- **Azure AI Foundry**: AI/ML services
- **Azure Functions**: Serverless compute

### CI/CD & Automation
- **GitHub Actions**: Automated workflows and deployments
- **CI/CD Pipelines**: Continuous integration and deployment
- **Automated Testing**: Integration with testing frameworks

---

## Version Control & Project Management

### Version Control
- **Git**: Distributed version control
- **GitHub**: Code hosting and collaboration
- **Git Flow**: Branching strategy for teams
- **Code Review**: Best practices and tools

### Agile & Project Management
- **Agile Methodology**: Iterative development
- **Scrum**: Sprint planning and execution
- **Jira**: Project tracking and management
- **GitHub Projects**: Integrated project management

---

## Testing & Quality Assurance

### Testing Frameworks
- **NUnit**: Unit testing for .NET
- **Unit Testing**: Test-driven development
- **Integration Testing**: End-to-end testing
- **Code Review**: Quality assurance practices

---

## Soft Skills

### Communication & Collaboration
- Strong communication skills in English and Norwegian
- Team collaboration and coordination
- Technical documentation and writing
- Teaching and mentoring abilities

### Problem-Solving & Organization
- Analytical thinking and problem-solving
- Time management and prioritization
- Goal-oriented approach
- Multi-tasking capabilities
- Highly organized workflow

### Adaptability & Learning
- Quick learner with new technologies
- Adaptable to changing requirements
- Resilient mindset
- Continuous improvement focus
