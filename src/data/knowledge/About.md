# About MD Safin Sarker

MD Safin Sarker is a Full Stack Developer with a solid foundation in both front-end and back-end development, and growing experience in applying AI techniques to real-world problems. He is passionate about building scalable, high-quality software solutions, with a continuous focus on learning, DevOps practices, and emerging technologies like machine learning and generative AI.

## Current Location
Based in Sandnes, Norway

## Professional Summary
Full Stack Developer and AI Engineer with expertise in building intelligent, scalable software solutions. Specialized in generative AI technologies including Stable Diffusion, LoRA fine-tuning, and RAG (Retrieval Augmented Generation) systems, while maintaining strong full-stack development capabilities across modern web technologies and cloud platforms.

Passionate about bridging the gap between traditional software engineering and cutting-edge AI/ML applications. Experienced in developing AI-powered applications, implementing DevOps practices, and creating production-ready solutions that leverage both classical software architecture and machine learning models.

## Personal Interests
- **Traveling**: Exploring new places and cultures
- **Bicycling**: Staying active and enjoying the outdoors
- **Playing Football**: Team sports and physical fitness

These activities help maintain work-life balance and contribute to overall well-being and creativity.

## Languages
- **English**: Fluent
- **Norwegian/Norsk**: Intermediate (B1 level)
- **Bengali**: Native

## Soft Skills and Organizational Abilities
- Time Management
- Goal-Oriented approach
- Problem Solving
- Multi-tasking capabilities
- Team Coordination
- Resilient mindset
- Highly Organized
- Skilled at evaluating options and generating solutions
- Able to coordinate several tasks simultaneously
